<template>
  <div id="Destroy" class="destroyTemplate">
    <div v-on:click="close()" class="close">X</div>
    <div class="title">BONTÁS</div>
    <div v-if="isClicked" class="verifyBox">{{verifyText}}
      <div class="row">
        <button class="submitButton fa fa-check" v-on:click="this.$emit('destroy',this.cField); this.isClicked=false;"></button>
    ...
    <div class="row justify-content-center">
      <div v-for="(prop,index) in game._fm._props" :key="prop" >
        <div v-if="prop._owner==name && isHaveFullGroup(prop._field,name) && prop._upgrades>0 && isOtherUpgradesOk(prop._field) && isHaveUpgradeMaterial(prop._field)">
...
export default {
  ...
  methods:{
    ...
    isHaveUpgradeMaterial(field){
      for(var i=0;i<this.game._fm._props.length;i++){
        if(this.game._fm._props[i]._field==field){
          if(this.game._fm._props[i]._upgrades==5){
            if(this.game._fm._stars>=4){
              return true;
            }else{
              return false;
            }
...
.destroyTemplate::-webkit-scrollbar {
  width: 15px;
  background-color:none;
  border-radius:10px;
}
.destroyTemplate::-webkit-scrollbar-thumb {
